(function($){
	...
	for(var i = 0;i<$('.menu-a').length;i++){
		aArr[i] = $('.menu-a').eq(i).text();
	}

	var idArr = [];
	for(var i = 0;i<$('.section').length;i++){
		idArr[i] = $('.section').eq(i).attr('id');
	}
	$(".menu-a").click(function(){
		var vThisE=$(this);
		// console.log(vThisE.text());
		//  console.log("test:"+this.parentNode);
		var akey = vThisE.text();
		function aNum(){
		...
		var pageActive = $('.active').eq(0).attr('id');
		function pNum(){
			for(var i = 0;i<idArr.length;i++){
				if (pageActive==idArr[i]) {
					return i;
				}
			}
		}
		if (aNum()>pNum()) {
			for(var i = 0;i<aNum()-pNum();i++)
			SP.moveSectionDown();
		}else if (aNum()<pNum()) {
			for(var i = 0;i<pNum()-aNum();i++)
			SP.moveSectionUp();
		}else{
			return;
		}
	})
	//webDesign 改变颜色 for section 4
	changeColor();
